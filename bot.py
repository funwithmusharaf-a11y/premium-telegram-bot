...
async def create_payment_message(message, uid, plan_id):
    ...
    asyncio.create_task(payment_timer(sent.chat.id, sent.message_id, payid, uid, plan_id))

def expired_payment_markup(uid, plan_id):
    text = (
...
async def payment_timer(chat_id, message_id, payid, uid, plan_id):
    # Updates every 5 seconds. At 00:00 it ONLY shows the expired screen.
    # A new QR is generated only when the user taps Retry Payment.
    while True:
        ...
        if remaining <= 0:
            ...
            db.commit()
            expired, kb = expired_payment_markup(uid, plan_id)
            try:
                await bot.edit_message_caption(chat_id, message_id, caption=expired, reply_markup=kb, parse_mode="HTML")
            except Exception:
                pass
            return
        try:
            ...
            # Message may have been deleted or replaced.
            return
...
async def cb_paid(c: CallbackQuery):
    ...
    if p["expires_at"] <= int(datetime.now(timezone.utc).timestamp()):
        # Expired payments must show the expired screen, not the old
        # "no longer pending" popup. Retry is the only way to create a new QR.
        db.execute("UPDATE payments SET status='expired' WHERE payment_id=? AND status='pending'", (payid,))
        db.commit()
        expired, kb = expired_payment_markup(c.from_user.id, p["plan_id"])
        try:
            await c.message.edit_caption(caption=expired, reply_markup=kb, parse_mode="HTML")
        except Exception:
            pass
        await c.answer()
        return
...
async def receive_document(m: Message):
    ...
    try:
        ...
        for table in ("users","plans","payments","settings"):
