...
async def choose_language(c: CallbackQuery):
    ...
    try:
        ...
        if media_count("start"):
            ...
            # Edit first; never delete the language message before editing it.
            await c.message.edit_text(full_text, reply_markup=user_plans_kb())
            save_user_media(c.from_user.id, [c.message.message_id])
    except Exception as e:
        print("Language callback error:", repr(e))
        try:
            await c.message.answer("⚠️ Something went wrong. Please try /start again.")
        except Exception:
            pass
...
async def plan_detail(c: CallbackQuery):
    # Answer immediately so Telegram never leaves the button in loading state.
    await c.answer()
    try:
        ...
        if media_count("plan", pid):
            # Album requires new messages. Remove the previous screen, then send album + card.
            await clear_user_media(c.from_user.id)
            ...
            # IMPORTANT: do NOT clear/delete the current message before editing it.
            # The previous version deleted it and then tried edit_text(), causing no response.
            await show_text_or_caption(c.message, text, kb, "HTML")
            save_user_media(c.from_user.id, [c.message.message_id])
    except Exception as e:
        print("Plan callback error:", repr(e))
        try:
            await c.message.answer("⚠️ Something went wrong. Please try /start again.")
        except Exception:
            pass
...
async def cb_plans(c: CallbackQuery):
    ...
    try:
        ...
        print("Plans callback error:", repr(e))
