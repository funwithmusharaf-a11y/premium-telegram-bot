...
async def show_admin(c: CallbackQuery):
    text = "⚙️ <b>Admin Panel</b>\\n\\nChoose an option:"
    kb = admin_menu()
    if c.message.photo:
        try:
            await c.message.edit_caption(caption=text, reply_markup=kb, parse_mode="HTML")
        except Exception:
...
async def doc(m:Message):
    ...
    try:
        ...
        # Copy validated backup data into the live database instead of replacing
        # the open SQLite connection/file underneath the running bot.
        srcdb = sqlite3.connect(tmp)
        srcdb.row_factory = sqlite3.Row
        db.execute("DELETE FROM users")
        db.execute("DELETE FROM plans")
        db.execute("DELETE FROM payments")
        db.execute("DELETE FROM settings")
        for r in srcdb.execute("SELECT id,username,first_name,language,created_at FROM users"):
            db.execute("INSERT INTO users(id,username,first_name,language,created_at) VALUES(?,?,?,?,?)",
                       tuple(r))
        for r in srcdb.execute("SELECT id,name,price,description,enabled FROM plans"):
            db.execute("INSERT INTO plans(id,name,price,description,enabled) VALUES(?,?,?,?,?)",
                       tuple(r))
        for r in srcdb.execute("SELECT id,payment_id,user_id,plan_id,amount,status,proof_file_id,created_at FROM payments"):
            db.execute("INSERT INTO payments(id,payment_id,user_id,plan_id,amount,status,proof_file_id,created_at) VALUES(?,?,?,?,?,?,?,?)",
                       tuple(r))
        for r in srcdb.execute("SELECT key,value FROM settings"):
            db.execute("INSERT INTO settings(key,value) VALUES(?,?)", tuple(r))
        db.commit()
        srcdb.close()
        os.remove(tmp)
        await m.answer("✅ Database imported successfully.", reply_markup=admin_menu())
...
async def main():
    ...
    print("Health server started on", PORT)
