...
async def clear_user_media(uid):
    rows = db.execute("SELECT message_id FROM user_media_messages WHERE user_id=?", (uid,)).fetchall()
    for r in rows:
        try:
            await bot.delete_message(uid, r["message_id"])
        except Exception as e:
            # Ignore already-deleted/expired messages; keep navigation working.
            if "message to delete not found" not in str(e).lower():
                print("Media delete error:", repr(e))
...
def save_user_media(uid, message_ids):
    ...
    db.executemany(
        ...
        [(uid, int(mid)) for mid in message_ids]
    )
    db.commit()
...
async def choose_language(c: CallbackQuery):
    ...
    if media_count("start"):
        try:
            ...
            save_user_media(c.from_user.id, media_ids)
