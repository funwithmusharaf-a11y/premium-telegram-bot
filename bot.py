...
async def create_payment_message(message, uid, plan_id):
    ...
    db.commit()
    task = asyncio.create_task(payment_timer(sent.chat.id, sent.message_id, payid, uid, plan_id))
    payment_timer_tasks.add(task)
    task.add_done_callback(payment_timer_tasks.discard)
...
async def payment_expiry_worker():
    """Global watcher: updates every pending payment and handles expiry even after restarts."""
    while True:
        try:
            rows = db.execute("SELECT * FROM payments WHERE status='pending' AND expires_at>0").fetchall()
            ...
            for p in rows:
                ...
                if remaining <= 0:
                    db.execute("UPDATE payments SET status='expired' WHERE payment_id=? AND status='pending'", (p["payment_id"],))
                    db.commit()
