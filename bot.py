# ============================================================
# MUSHU SINGLE-FILE BOT
# Put your BOT_TOKEN and OWNER_ID in the CONFIG section below.
# UPI is already configured. No .env / requirements.txt needed.
# Railway start command: python mushu.py
# ============================================================
...
# Auto-install dependencies so this can run as a single Python file.
_REQUIRED = {
    "aiogram": "aiogram>=3.31,<4",
    "aiohttp": "aiohttp>=3.9",
...
if not TOKEN or TOKEN == "PASTE_BOT_TOKEN_HERE":
    raise RuntimeError("BOT_TOKEN is missing. Set BOT_TOKEN in the CONFIG section of mushu.py.")
if not OWNER_ID:
    raise RuntimeError("OWNER_ID is missing. Set OWNER_ID in the CONFIG section of mushu.py.")
...
async def choose_language(c: CallbackQuery):
    # Acknowledge the button immediately so Telegram stops showing the loading spinner.
    await c.answer()
    ...
    if media_count("start"):
        try:
            # Delete the current language message directly; avoid a slow full-screen
            # cleanup here, which can make the language buttons appear unresponsive.
            try:
                await c.message.delete()
            except Exception:
                pass
            db.execute("DELETE FROM user_media_messages WHERE user_id=?", (c.from_user.id,))
            db.execute("DELETE FROM user_screen_messages WHERE user_id=?", (c.from_user.id,))
            db.commit()
        ...
        # No media: edit the same message instead of doing a blocking cleanup.
...
async def plan_detail(c: CallbackQuery):
    # Acknowledge FIRST so Telegram never leaves the plan button spinning.
    await c.answer()
    try:
    ...
    # If this plan has no media, edit the existing plan-list message directly.
    # Do NOT delete it first: editing a deleted message is what caused the
    # previous "no response" behaviour.
    if not media_count("plan", pid):
        try:
            await c.message.edit_text(text, reply_markup=kb, parse_mode="HTML")
        except Exception as e:
            print("Plan edit error:", repr(e))
            try:
                msg = await bot.send_message(c.from_user.id, text, reply_markup=kb, parse_mode="HTML")
                save_user_media(c.from_user.id, [msg.message_id])
                return
            except Exception as e2:
                print("Plan send fallback error:", repr(e2))
                return
        save_user_media(c.from_user.id, [c.message.message_id])
        return

    # With media, remove the old screen and then send the album + details.
    ids = set()
    for table in ("user_media_messages", "user_screen_messages"):
        rows = db.execute(f"SELECT message_id FROM {table} WHERE user_id=?", (c.from_user.id,)).fetchall()
        ids.update(int(r["message_id"]) for r in rows)

    async def delete_one(mid):
        try:
            await bot.delete_message(chat_id=c.from_user.id, message_id=mid)
        except Exception:
            pass
    ...
    try:
        ...
        text_msg = await bot.send_message(c.from_user.id, text, reply_markup=kb, parse_mode="HTML")
        save_user_media(c.from_user.id, media_ids + [text_msg.message_id])
    except Exception as e:
        print("Plan media layout error:", repr(e))
        try:
            text_msg = await bot.send_message(c.from_user.id, text, reply_markup=kb, parse_mode="HTML")
            save_user_media(c.from_user.id, [text_msg.message_id])
        except Exception as e2:
            print("Plan media fallback error:", repr(e2))
    ...
    try:
        ...
        if st.startswith("planedit:"):
            ...
            if field == "price":
                ...
                db.execute(f"UPDATE plans SET {field if field in ('name','button_text','description_en','description_hi') else 'name'}=? WHERE id=?", (text,pid))
            db.commit()
            state.pop(m.from_user.id, None)
