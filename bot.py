...
async def send_media_set(chat_id, scope, plan_id=None):
    ...
    if len(rows) == 1:
        ...
        if r["media_type"] == "photo":
            await bot.send_photo(chat_id, r["file_id"])
        else:
            await bot.send_video(chat_id, r["file_id"])
        return
    items = []
    for r in rows:
        if r["media_type"] == "photo":
            items.append(InputMediaPhoto(media=r["file_id"]))
        else:
            items.append(InputMediaVideo(media=r["file_id"]))
...
async def choose_language(c: CallbackQuery):
    ...
    await send_media_set(c.from_user.id, "start")
