...
async def send_media_set(chat_id, scope, plan_id=None, caption=None, parse_mode=None):
    """Send up to 6 media items as a Telegram media group.

    When a caption is supplied, it is attached to the first media item.
    Telegram media groups cannot have an inline keyboard attached to the
    whole album, so callers can send the keyboard as the next message.
    """
    ...
    if len(rows) == 1:
        ...
        if r["media_type"] == "photo":
            await bot.send_photo(chat_id, r["file_id"], caption=caption, parse_mode=parse_mode)
        else:
            await bot.send_video(chat_id, r["file_id"], caption=caption, parse_mode=parse_mode)
        return True
    ...
    await bot.send_media_group(chat_id, media=items)
    return True
...
async def cmd_start(m: Message):
    # Notify the owner only when this Telegram user starts the bot for the first time.
...
async def choose_language(c: CallbackQuery):
    ...
    if media_count("start"):
        try:
            ...
            sent_media = await send_media_set(c.from_user.id, "start")
            await bot.send_message(c.from_user.id, full_text, reply_markup=user_plans_kb())
        except Exception as e:
...
async def cb_start(c: CallbackQuery):
    await show_text_or_caption(
        c.message,
        "🌐 Please choose your language /\nकृपया अपनी भाषा चुनें:",
        InlineKeyboardMarkup(inline_keyboard=[[btn("🇮🇳 हिंदी","lang:hi"),btn("🇬🇧 English","lang:en")]])
    )
    await c.answer()
...
async def plan_detail(c: CallbackQuery):
    ...
    if media_count("plan", pid):
        try:
            ...
            pass
        try:
            await send_media_set(c.from_user.id, "plan", pid)
...
async def cb_plans(c: CallbackQuery):
    await show_text_or_caption(c.message, user_text(c.from_user.id, "plans"), user_plans_kb())
    await c.answer()
...
async def receive_document(m: Message):
    ...
    try:
        ...
        for table in ("users","plans","payments","settings","media"):
