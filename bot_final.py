...
async def clear_user_media(uid):
    """Delete all tracked welcome/plan screen messages quickly.

    Media albums contain several Telegram messages, so every tracked message_id
    is deleted. Deletions run concurrently so changing language/navigation does
    not leave the Telegram callback spinner running for a long time.
    """
    ids = set()
    ...
    async def delete_one(message_id):
        try:
            await asyncio.wait_for(
                bot.delete_message(chat_id=uid, message_id=message_id),
                timeout=4,
            )
    ...
    if ids:
        await asyncio.gather(*(delete_one(mid) for mid in ids), return_exceptions=True)
...
async def cmd_start(m: Message):
    # Register the user atomically. Only the first /start for a Telegram ID
    # sends the owner "New User" notification.
    is_new_user = False
    try:
        db.execute("BEGIN IMMEDIATE")
        existing = db.execute(
            "SELECT id FROM users WHERE id=?", (m.from_user.id,)
        ).fetchone()
...
async def choose_language(c: CallbackQuery):
    ...
    # Required order: welcome media first, then the text/options message.
    if media_count("start"):
        ...
        try:
            ...
            try:
                ...
                print("Start screen fallback error:", repr(e2))
    else:
        await clear_user_media(c.from_user.id)
        try:
            await c.message.edit_text(
                full_text, reply_markup=user_plans_kb()
            )
