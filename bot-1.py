...
def detach_user_screen_messages(uid):
    """Atomically detach the old welcome/plan message IDs from tracking.
    The Telegram deletions are performed separately so /start never waits for
    a batch of old media deletions before showing the language screen.
    """
    ids = set()
...
async def delete_message_ids(uid, ids):
    """Delete detached screen messages without blocking the new screen."""
    ...
    if ids:
        await asyncio.gather(*(_delete_one(mid) for mid in ids))

async def clear_user_media(uid):
    ids = detach_user_screen_messages(uid)
    await delete_message_ids(uid, ids)
...
async def cmd_start(m: Message):
    ...
    # IMPORTANT: never wait for deletion of old media before replying to /start.
    # On slow/free hosts that can make the bot look completely dead. Detach the
    # old IDs immediately, show the language picker, then delete old messages
    # in the background. The new language message is already tracked separately.
    old_ids = detach_user_screen_messages(m.from_user.id)
    ...
    if old_ids:
        asyncio.create_task(delete_message_ids(m.from_user.id, old_ids))

@dp.callback_query(F.data.startswith("lang:"))
async def choose_language(c: CallbackQuery):
    # Acknowledge immediately so Telegram never leaves the language button
    # in a loading/spinner state while old media messages are cleaned up.
    await c.answer()
    ...
    # Remove the old language screen and any previous welcome/plan media.
    try:
        await clear_user_media(c.from_user.id)
    except Exception as e:
        print("Language screen cleanup error:", repr(e))

    try:
        media_ids = await send_media_set(c.from_user.id, "start")
        text_msg = await bot.send_message(
            c.from_user.id, full_text, reply_markup=user_plans_kb()
        )
        save_user_media(c.from_user.id, media_ids + [text_msg.message_id])
    except Exception as e:
        print("Language selection screen error:", repr(e))
        try:
            text_msg = await bot.send_message(
                c.from_user.id, full_text, reply_markup=user_plans_kb()
            )
            ...
            print("Language fallback error:", repr(e2))
